...
export default function MobileBroilerApp() {
  ...
  const companyId = useMemo(() => {
    ...
    const raw = window.localStorage.getItem("ovicore_selected_company_id");
  ...
  const loadData = useCallback(async () => {
    if (!companyId) {
      setLoading(false);
      setMessage("Select a working company in OviCore first.");
      return;
    }
  ...
  }, [loadData, loadPendingCount, syncDrafts]);
