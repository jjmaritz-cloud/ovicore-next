...
const USER_ACTIVITY_SUMMARY_ENDPOINT = `${API_BASE}/api/access/users/activity-summary`;
...
function parseApiDate(value: string | null | undefined) {
  ...
  // FastAPI currently serialises datetime.utcnow() values without a timezone
  // suffix. JavaScript interprets those strings as local time, so explicitly
  // mark timezone-less API timestamps as UTC before displaying them.
...
function formatLocalDateTime(value: string | null | undefined, fallback = "—") {
  const date = parseApiDate(value);
  return date ? date.toLocaleString() : fallback;
}

function dateFormatter(params: ValueFormatterParams) {
  const date = parseApiDate(params.value);
  return date ? date.toLocaleDateString() : "";
}

function dateTimeFormatter(params: ValueFormatterParams) {
  return formatLocalDateTime(params.value, "Never");
}

function usageStatusClass(status?: string) {
  if (status === "Active") return "ovicore-pill ovicore-pill-green";
  if (status === "Low Usage") return "ovicore-pill ovicore-pill-amber";
  return "ovicore-pill";
}
...
export default function AdminUsersPage() {
  ...
  const [currentUserIsGlobalAdmin, setCurrentUserIsGlobalAdmin] = useState(false);
  ...
  const fetchCurrentUser = useCallback(async () => {
    ...
    const data: { is_global_admin?: boolean } = await response.json();
    const isGlobalAdmin = data.is_global_admin === true;
    setCurrentUserIsGlobalAdmin(isGlobalAdmin);
  ...
  const fetchCompanies = useCallback(async () => {
	...
	const fetchUsers = useCallback(async (loadedCompanies: CompanyRow[], isGlobalAdmin: boolean) => {
		...
		let activitySummary: UserActivitySummary[] = [];
		if (isGlobalAdmin) {
			try {
				const activityResponse = await authenticatedFetch(
					USER_ACTIVITY_SUMMARY_ENDPOINT,
					{ cache: "no-store" }
				);

				if (activityResponse.ok) {
					activitySummary = await activityResponse.json();
				}
			} catch (error) {
				console.debug("User activity summary unavailable.", error);
			}
		}

		const activityByUserId = new Map(
			activitySummary.map((item) => [item.user_id, item])
		);
	...
	const fetchRows = useCallback(async () => {
		...
		try {
			const isGlobalAdmin = await fetchCurrentUser();
			const loadedCompanies = await fetchCompanies();
			await fetchUsers(loadedCompanies, isGlobalAdmin);
		} catch (error) {
			console.error(error);
	...
	}, [fetchCompanies, fetchCurrentUser, fetchUsers]);

  const loadActivityForUser = useCallback(async (user: UserRow | null) => {
    setActivityEvents([]);
    if (!user || !currentUserIsGlobalAdmin) return;

    setLoadingActivity(true);
    try {
      const response = await authenticatedFetch(
        `${USERS_ENDPOINT}/${user.id}/activity?limit=50`,
        { cache: "no-store" }
      );
      ...
      if (response.ok) {
        ...
        console.error(`Could not load activity. ${response.status}`);
      }
    } catch (error) {
      console.error("Could not load user activity.", error);
    } finally {
  ...
  const activeLast7DaysCount = useMemo(
    () => rows.filter((row) => row.usage_status === "Active").length,
    [rows]
  );

  const dormantUserCount = useMemo(
    () => rows.filter((row) => row.usage_status === "Dormant").length,
    [rows]
  );

  const neverLoggedInCount = useMemo(
    () => rows.filter((row) => row.usage_status === "Never Logged In").length,
    [rows]
  );
  ...
  const columnDefs = useMemo<ColDef<UserRow>[]>(
    () => [
      ...
      ...(currentUserIsGlobalAdmin
        ? [
            {
              field: "last_login_at" as keyof UserRow,
            ...
            {
              field: "last_active_at" as keyof UserRow,
            ...
            {
              field: "sessions_30d" as keyof UserRow,
              headerName: "30d Sessions",
              editable: false,
              minWidth: 135,
            },
            {
              field: "last_module" as keyof UserRow,
              headerName: "Last Module",
              editable: false,
              minWidth: 145,
            },
            {
              field: "usage_status" as keyof UserRow,
              headerName: "Usage",
              editable: false,
              minWidth: 145,
              cellRenderer: (params: ICellRendererParams<UserRow>) => (
                <span className={usageStatusClass(params.value)}>
                  {params.value || "—"}
                </span>
              ),
            },
    ...
    [companyNameOptions, currentUserIsGlobalAdmin, deleteUser, resetUserPassword]
  );
  ...
  return (
    <OviCoreShell module="admin">
        ...
        items={
          currentUserIsGlobalAdmin
            ? [
                ...
                { label: "Enabled", value: activeUserCount },
                { label: "Active 7d", value: activeLast7DaysCount },
                { label: "Dormant", value: dormantUserCount },
      ...
      {currentUserIsGlobalAdmin ? (
        ...
        <OviCoreTableCard
          title="User Activity"
          subtitle="Global Admin usage view. Activity is limited to logins and meaningful page changes; heartbeats only refresh Last Active."
        >
          {!selectedUser ? (
            <div style={{ padding: 18, color: "var(--ovicore-muted)", textAlign: "center" }}>
              Select a user above to view their activity.
            </div>
          ) : (
            <>
              <div
              ...
              >
                {[
                  ["Last login", formatLocalDateTime(selectedUser.last_login_at, "Never")],
                  ["Last active", formatLocalDateTime(selectedUser.last_active_at)],
                  ["30d sessions", String(selectedUser.sessions_30d ?? 0)],
                  ["Last module", selectedUser.last_module || "—"],
                  ["Last page", selectedUser.last_page || "—"],
                ].map(([label, value]) => (
                  <div
                    key={label}
                    style={{
                      border: "1px solid var(--ovicore-border)",
                      borderRadius: 10,
                      padding: "10px 12px",
                      background: "#fff",
                    }}
                  >
                    <div style={{ fontSize: 10, fontWeight: 850, textTransform: "uppercase", color: "var(--ovicore-muted)" }}>
                      {label}
                    </div>
                    <div style={{ marginTop: 4, fontSize: 13, fontWeight: 750, color: "var(--ovicore-green-900)", overflowWrap: "anywhere" }}>
                      {value}
                    </div>
                  </div>
                ))}
              ...
              {loadingActivity ? (
                <div style={{ padding: 14, color: "var(--ovicore-muted)" }}>Loading activity…</div>
              ) : activityEvents.length === 0 ? (
                <div style={{ padding: 14, color: "var(--ovicore-muted)" }}>No activity history recorded yet.</div>
              ) : (
                <div style={{ display: "grid", gap: 6, maxHeight: 260, overflowY: "auto" }}>
                  {activityEvents.map((event) => (
                    <div
                      ...
                      style={{
                        display: "grid",
                        gridTemplateColumns: "150px 110px 150px 1fr",
                        gap: 10,
                        alignItems: "center",
                        padding: "8px 10px",
                        borderBottom: "1px solid var(--ovicore-border)",
                        fontSize: 12,
                      }}
                    >
                      ...
                      <span style={{ color: "var(--ovicore-muted)", overflowWrap: "anywhere" }}>{event.page || "—"}</span>
                    </div>
                  ))}
                </div>
              )}
