...
title OviCore User Activity Update - V4
...
echo.
echo ============================================================
echo   OviCore User Activity Update - V4
echo ============================================================
echo.
...
rem ------------------------------------------------------------
rem Find live frontend files safely
rem ------------------------------------------------------------
set "AUTHGATE="
...
if not defined AUTHGATE (
  echo [ERROR] Could not find the LIVE AuthGate.tsx anywhere under:
  echo   %PROJECT%\frontend\src
  echo.
  echo Run this command and send me the result:
  echo   where /r "%PROJECT%\frontend\src" AuthGate.tsx
  pause
  exit /b 1
)

if not exist "%USERPAGE%" (
  echo [ERROR] Could not find the Users page:
  echo   %USERPAGE%
  pause
  exit /b 1
)
...
rem ------------------------------------------------------------
rem Check package files before doing anything
rem ------------------------------------------------------------
...
mkdir "%BACKUP%\backend\app\routes" >nul 2>&1
...
call :backup "%PROJECT%\backend\app\routes\auth.py" "%BACKUP%\backend\app\routes\auth.py" || goto :fail
call :backup "%PROJECT%\backend\app\routes\access.py" "%BACKUP%\backend\app\routes\access.py" || goto :fail
...
call :install "%PKG%auth.py" "%PROJECT%\backend\app\routes\auth.py" || goto :fail
call :install "%PKG%access.py" "%PROJECT%\backend\app\routes\access.py" || goto :fail
...
echo   %BACKUP%
echo.
echo Restart the OviCore backend and frontend, then log out and in.
echo.
pause
exit /b 0
