...
@router.get(
    ...
    response_model=list[schemas.UserActivitySummaryOut],
)
def list_user_activity_summary(
    ...
    session_counts = dict(
        ...
        .filter(
            models.UserActivity.event_type == "login",
            models.UserActivity.created_at >= cutoff,
        )
    ...
    for user in users:
        ...
        if user.last_login_at is None:
            usage_status = "Never Logged In"
            days_since_active = None
        elif last_seen is None:
            usage_status = "Dormant"
            days_since_active = None
        else:
            days_since_active = max((now - last_seen).days, 0)
        ...
        result.append(
            schemas.UserActivitySummaryOut(
                user_id=user.id,
                last_login_at=user.last_login_at,
                last_active_at=user.last_active_at,
                last_module=user.last_module,
                last_page=user.last_page,
                sessions_30d=int(session_counts.get(user.id, 0)),
...
@router.get(
    "/users/{target_user_id}/activity",
    response_model=list[schemas.UserActivityOut],
)
def list_user_activity(
    ...
    return (
        ...
        .filter(models.UserActivity.user_id == target_user_id)
